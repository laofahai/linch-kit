...
### 🔴 强制执行清单
每次任务开始，必须完成以下项目，违规即停：
1. **自动触发 /start** - 不是建议，是命令，拒绝执行未经 /start 分析的任务
2. **Graph RAG 查询** - 跳过即违规，立即停止所有开发活动
3. **包复用检查** - 发现重复实现，强制删除代码，无例外
4. **架构合规验证** - 违规即回滚全部更改，重新开始

### 💀 违规即死条款
- **第1次违规**: 立即停止当前操作，强制从第一步重来
- **第2次违规**: 会话终止，用户必须重新开始新会话
- **任何借口**: 一律无效，技术理由不接受，执行为唯一标准

### ⚡ 强制流程协议
```
用户提出任务 → 立即执行 /start → 基于结果执行约束检查 → 违规即停止
```
...
- **每次代码编写**: 必须确认无重复实现
- **每个功能开发**: 必须通过架构检查
- **验证失败**: 立即回滚，强制重做，不接受部分完成

**执行标准：违规即停，纠正即续，再违规即终止会话。**
