...
export const TenantEntity = defineEntity('Tenant', {
  // 基础字段
  name: defineField.string().required().unique().max(100).description('租户名称'),

  domain: defineField.string().unique().max(255).optional().description('自定义域名'),

  slug: defineField.string().required().unique().max(50).description('URL友好标识符'),

  description: defineField.text().optional().description('租户描述'),

  businessLicense: defineField.string().optional().max(50).description('营业执照号码'),
  ...
  billingCycle: defineField.enum(['monthly', 'yearly']).optional().description('计费周期'),

  maxUsers: defineField.int().required().default(10).min(1).description('最大用户数限制'),
  ...
  // 扩展字段
  settings: defineField.json<TenantSettings>().default({}).description('租户特定设置'),

  metadata: defineField.json<TenantMetadata>().default({}).description('扩展元数据'),
  ...
  deletedAt: defineField.datetime().optional().description('软删除时间'),
})
...
export const TenantQuotasEntity = defineEntity('TenantQuotas', {
  // 关系字段
  tenant: defineField.relation('Tenant').oneToOne().required().description('关联租户'),

  // 用户配额
  maxUsers: defineField.int().required().default(10).min(0).description('最大用户数'),

  currentUsers: defineField.int().required().default(0).min(0).description('当前用户数'),
  ...
  currentApiCalls: defineField.int().required().default(0).min(0).description('当前API调用次数'),

  apiResetAt: defineField.datetime().optional().description('API配额重置时间'),

  // 插件配额
  maxPlugins: defineField.int().required().default(5).min(0).description('最大插件数'),

  currentPlugins: defineField.int().required().default(0).min(0).description('当前插件数'),

  // 数据模型配额
  maxSchemas: defineField.int().required().default(10).min(0).description('最大Schema数'),

  currentSchemas: defineField.int().required().default(0).min(0).description('当前Schema数'),
