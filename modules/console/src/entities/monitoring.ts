...
export const AuditLogEntity = defineEntity('AuditLog', {
  ...
  category: defineField.enum([
    'SECURITY', 'DATA', 'SYSTEM', 'BUSINESS', 'PERMISSION', 'PLUGIN'
  ], {
    required: true,
    defaultValue: 'SYSTEM',
    description: '审计分类'
...
export const NotificationEntity = defineEntity('Notification', {
  ...
  type: defineField.enum([
    'ALERT', 'INFO', 'WARNING', 'SUCCESS', 'MAINTENANCE'
  ], {
    required: true,
    description: '通知类型'
  }),
  ...
  // 通知渠道
  channels: defineField.array(
    defineField.enum(['WEB', 'EMAIL', 'SMS', 'WEBHOOK']), 
    {
      description: '通知渠道'
    }
  ),
...
export const AlertRuleEntity = defineEntity('AlertRule', {
  ...
  // 通知配置
  notificationChannels: defineField.array(
    defineField.enum(['WEB', 'EMAIL', 'SMS', 'WEBHOOK']), 
    {
      required: true,
      description: '通知渠道'
