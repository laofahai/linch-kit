 ...
 * Console 路由渲染组件
 */
export function ConsoleRoutes({ config, params, permissions = [], tenantId }: ConsoleRoutesProps) {
...
export function ConsoleRouter({
  ...
  const content = (
    <div className="console-router">
      {showBreadcrumbs && breadcrumbs.length > 0 && <ConsoleBreadcrumbs items={breadcrumbs} />}
      <ConsoleRoutes {...props} />
    </div>
  )
        ...
        {items.map((item, index) => (
          <li key={item.id} className="inline-flex items-center">
            ...
            {index === items.length - 1 ? (
              ...
              >
                {item.icon && <span className="mr-2 w-4 h-4">{/* Icon component */}</span>}
                {item.title}
              </a>
            )}
          </li>
