/**
 * @linch-kit/schema 装饰器系统完整实现
 *
 * 提供基于装饰器的Schema定义方式，支持类和属性装饰器
 *
 * @module decorators/minimal
 */
...
// 类型定义
type FieldBuilder<T extends BaseFieldDefinition = FieldDefinition> = {
  build(): T
}
type PropertyDecorator = (target: Record<string, unknown>, propertyKey: string) => void
...
export function Entity(name?: string, options?: EntityOptions) {
  return function <T extends new (...args: any[]) => Record<string, unknown>>(target: T) {
 ...
 * 从装饰器类创建实体
 */
export function getEntityFromClass(target: any): EntityType {
 ...
 * 批量从装饰器类创建实体
 */
export function getEntitiesFromClasses(classes: any[]): Record<string, EntityType> {
