...
export class ContextAnalyzer implements IContextAnalyzer {
  ...
  async extractRelevantNodes(
    ...
    const nodeScores = nodes.map(node => {
      ...
      // 1. 名称匹配
      if (node.name && promptWords.some(word => 
        node.name.toLowerCase().includes(word) || word.includes(node.name.toLowerCase())
      )) {
        score += 0.8
        reasons.push('名称匹配')
      }
  ...
  private async identifyPatterns(
    ...
    if (promptLower.includes('auth') || promptLower.includes('login') || promptLower.includes('user')) {
      const authNodes = relevantNodes.filter(item => 
        item.node.package === '@linch-kit/auth' || 
        item.node.name?.toLowerCase().includes('auth')
      )
    ...
    // CRUD 模式
    if (promptLower.includes('crud') || promptLower.includes('create') || promptLower.includes('database')) {
      const crudNodes = relevantNodes.filter(item => 
        item.node.package === '@linch-kit/crud'
      )
  ...
  private async generateRecommendations(
    ...
    // 架构建议
    const hasLinchKitNodes = relevantNodes.some(item => 
      item.node.package && this.linchkitPackages.has(item.node.package)
    )
  ...
  private arePackagesConflicting(pkg1: string, pkg2: string): boolean {
    ...
    return conflictPairs.some(([a, b]) => 
      (pkg1.includes(a) && pkg2.includes(b)) || 
      (pkg1.includes(b) && pkg2.includes(a))
    )
  }
}