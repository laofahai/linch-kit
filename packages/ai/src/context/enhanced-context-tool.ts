...
export class EnhancedContextTool {
  ...
  async queryEnhancedContext(
    ...
    try {
      ...
      // 5. 生成AI建议
      const suggestions = this.generateSuggestions(queryAnalysis, entityDefinition, options)
  ...
  private generateSuggestions(
    ...
    // 生成字段建议
    if (
      queryAnalysis.detected_action === DetectedAction.ADD_FIELD &&
    ...
    // 生成潜在影响
    suggestions.potential_impacts = this.generatePotentialImpacts(queryAnalysis, entityDefinition)
   ...
   * 生成字段建议
   */
  private generateFieldSuggestion(fieldName: string, _entityName?: string): FieldSuggestion {
    // 根据字段名推断类型
    const typeMapping = {
