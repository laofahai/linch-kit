...
export class RequirementAnalyzer {
    ...
    this.intentPatterns = [
      {
        ...
        confidence_base: 0.9,
        scope_hints: [
          DevelopmentScope.SCHEMA,
          DevelopmentScope.DATABASE,
          DevelopmentScope.API,
          DevelopmentScope.UI,
        ],
      },
      {
        intent: DevelopmentIntent.REMOVE_FIELD,
        patterns: [/删除.*字段/i, /移除.*字段/i, /去掉.*属性/i, /remove.*field/i, /delete.*field/i],
        confidence_base: 0.9,
        scope_hints: [
          DevelopmentScope.SCHEMA,
          DevelopmentScope.DATABASE,
      ...
      },
      {
        ...
        patterns: [/创建.*API/i, /新建.*接口/i, /add.*endpoint/i, /create.*route/i, /.*需要.*API/i],
        confidence_base: 0.8,
      ...
      },
      {
        ...
        patterns: [/添加.*验证/i, /.*校验/i, /add.*validation/i, /validate.*input/i],
        confidence_base: 0.7,
   ...
   * 提取字段需求
   */
  private extractFieldRequirement(
    userInput: string,
    intent: { intent: DevelopmentIntent }
