...
describe('PluginRegistry', () => {
  ...
  describe('Plugin Registration', () => {
    ...
    it('should reject duplicate plugin registration', () => {
      ...
      expect(() => registry.registerSync(mockPlugin)).toThrow('Plugin test-plugin is already registered')
    })
    ...
    it('should validate plugin required methods', () => {
      ...
      expect(() => registry.registerSync(invalidPlugin)).toThrow('Plugin must implement required lifecycle methods')
    })
  ...
  describe('Dependency Management', () => {
    ...
    it('should detect circular dependencies', async () => {
      const pluginA = { ...mockPlugin, metadata: { ...mockPlugin.metadata, id: 'plugin-a', dependencies: ['plugin-b'] } }
      const pluginB = { ...mockPlugin, metadata: { ...mockPlugin.metadata, id: 'plugin-b', dependencies: ['plugin-a'] } }
    ...
    it('should fail when dependency is missing', async () => {
      const pluginWithMissingDep = {
        ...mockPlugin,
        metadata: { ...mockPlugin.metadata, id: 'missing-dep-plugin', dependencies: ['non-existent'] }
      }
      
      registry.registerSync(pluginWithMissingDep)
      
      await expect(registry.startPlugin('missing-dep-plugin')).rejects.toThrow('Dependency non-existent not found')
    })
  })
  ...
  describe('Plugin Metadata Validation', () => {
    ...
    it('should validate dependency format', () => {
      const invalidDepsPlugin = {
        ...mockPlugin,
        metadata: { ...mockPlugin.metadata, id: 'invalid-deps', dependencies: 'not-an-array' as any } // Should be array
      }
