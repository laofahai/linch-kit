...
describe('DefaultDataMasker', () => {
  ...
  describe('邮箱脱敏', () => {
    it('should mask email addresses correctly', () => {
      expect(dataMasker.maskValue('john.doe@example.com', 'email'))
        .toBe('j***e@example.com')
      
      expect(dataMasker.maskValue('a@test.com', 'email'))
        .toBe('***@test.com')
      
      expect(dataMasker.maskValue('ab@test.com', 'email'))
        .toBe('***@test.com')
      
      expect(dataMasker.maskValue('test@domain.org', 'email'))
        .toBe('t***t@domain.org')
    })
    ...
    it('should handle invalid email formats', () => {
      ...
      expect(dataMasker.maskValue(123, 'email'))
        .toBe('1***3')
    })
  })
  ...
  describe('电话号码脱敏', () => {
    it('should mask phone numbers correctly', () => {
      expect(dataMasker.maskValue('1234567890', 'phone'))
        .toBe('******7890')
      
      expect(dataMasker.maskValue('(123) 456-7890', 'phone'))
        .toBe('(***) ***-7890')
      
      expect(dataMasker.maskValue('+1-234-567-8901', 'phone'))
        .toBe('+*-***-***-8901')
    })

    it('should handle short phone numbers', () => {
      expect(dataMasker.maskValue('123', 'phone'))
        .toBe('***')
      
      expect(dataMasker.maskValue('1234', 'phone'))
        .toBe('1234')
    })
  ...
  describe('信用卡号脱敏', () => {
    it('should mask credit card numbers correctly', () => {
      expect(dataMasker.maskValue('1234567812345678', 'creditCard'))
        .toBe('1234********5678')
      
      expect(dataMasker.maskValue('4111-1111-1111-1111', 'creditCard'))
        .toBe('4111********1111')
    })

    it('should handle short card numbers', () => {
      expect(dataMasker.maskValue('123', 'creditCard'))
        .toBe('***')
  ...
  describe('SSN 脱敏', () => {
    it('should mask SSN correctly', () => {
      expect(dataMasker.maskValue('123456789', 'ssn'))
        .toBe('***-**-6789')
      
      expect(dataMasker.maskValue('123-45-6789', 'ssn'))
        .toBe('***-**-6789')
    })

    it('should handle invalid SSN formats', () => {
      expect(dataMasker.maskValue('12345', 'ssn'))
        .toBe('1***5')
  ...
  describe('默认脱敏策略', () => {
    it('should mask strings correctly', () => {
      expect(dataMasker.maskValue('secret123', 'password'))
        .toBe('se***23')
      
      expect(dataMasker.maskValue('ab', 'password'))
        .toBe('***')
      
      expect(dataMasker.maskValue('short', 'password'))
        .toBe('s***t')
      
      expect(dataMasker.maskValue('verylongpassword', 'password'))
        .toBe('ve***rd')
    })

    it('should mask numbers correctly', () => {
      expect(dataMasker.maskValue(123456, 'secretNumber'))
        .toBe('1***6')
      
      expect(dataMasker.maskValue(42, 'secretNumber'))
        .toBe('***')
      
      expect(dataMasker.maskValue(123, 'secretNumber'))
        .toBe('1***3')
    })
  ...
  describe('数组脱敏', () => {
    ...
    it('should handle nested arrays', () => {
      const nested = [['secret1', 'secret2'], ['secret3', 'secret4']]
      const masked = dataMasker.maskArray(nested)
      
      expect(masked).toEqual([['secret1', 'secret2'], ['secret3', 'secret4']])
    })
  ...
  describe('非敏感字段', () => {
    it('should not mask non-sensitive fields', () => {
      const nonSensitiveFields = [
        'username', 'name', 'age', 'status', 'role', 'department',
  ...
  describe('边界情况', () => {
    ...
    it('should handle special characters', () => {
      expect(dataMasker.maskValue('p@$$w0rd!', 'password'))
        .toBe('p@***d!')
      
      expect(dataMasker.maskValue('émâil@tëst.cöm', 'email'))
        .toBe('é***l@tëst.cöm')
    })
