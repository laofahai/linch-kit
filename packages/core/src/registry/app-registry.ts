/**
 * 应用注册器 - 运行时注册和管理应用级资源
 * @module core/registry/app-registry
 */
...
export class AppRegistry extends EventEmitter implements IAppRegistry {
  protected models = new Map<string, ModelDefinition>()
  protected modelExtensions = new Map<string, ModelExtension[]>()
  protected routes = new Map<string, RouteRegistration>()
  protected components = new Map<string, ComponentRegistration>()
  protected pageRoutes = new Map<string, PageRouteRegistration>()
  protected menus = new Map<string, MenuRegistration>()
  protected trpcRouter?: unknown
   ...
   * 设置tRPC路由器实例
   */
  setTrpcRouter(router: unknown): void {
   ...
   * 获取当前tRPC路由器
   */
  getTrpcRouter(): unknown | undefined {
    ...
    for (const ext of extensions) {
      ...
      if (ext.relations && extendedModel.relations) {
        extendedModel.relations = [
          ...extendedModel.relations,
          ...(ext.relations as unknown as unknown[]),
        ]
      }
