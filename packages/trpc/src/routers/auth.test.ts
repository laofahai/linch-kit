...
describe('@linch-kit/trpc Auth Router', () => {
  ...
  describe('getSession 会话获取', () => {
    it('should return null for unauthenticated users', async () => {
      const caller = authRouter.createCaller(mockPublicContext)
    ...
    it('should handle different user types', async () => {
      const caller = authRouter.createCaller(mockAdminContext)
  ...
  describe('isAuthenticated 认证状态检查', () => {
    ...
    it('should be accessible without authentication', async () => {
      ...
      // 应该不抛出认证错误
      await expect(caller.isAuthenticated()).resolves.toBe(false)
    })
  ...
  describe('getUser 用户信息获取', () => {
    ...
    it('should require authentication', async () => {
      ...
      await expect(caller.getUser()).rejects.toThrow('需要登录才能访问此资源')
    })
  ...
  describe('getPermissions 权限获取', () => {
    ...
    it('should require authentication', async () => {
      ...
      await expect(caller.getPermissions()).rejects.toThrow('需要登录才能访问此资源')
    })
    ...
    it('should handle edge cases', async () => {
      ...
      const caller = authRouter.createCaller(contextWithMinimalUser)
      const result = await caller.getPermissions()
      expect(result).toEqual([])
    })
  })
  ...
  describe('hasPermission 权限检查', () => {
    it('should accept valid input schema', async () => {
      ...
      const result = await caller.hasPermission(validInput)
      expect(result).toBe(false)
    })
    ...
    it('should require authentication', async () => {
      ...
      await expect(caller.hasPermission({ action: 'read', resource: 'posts' })).rejects.toThrow(
        '需要登录才能访问此资源'
      )
    })
    ...
    it('should handle different action types', async () => {
      ...
      for (const action of actions) {
        const result = await caller.hasPermission({ action, resource: 'posts' })
    ...
    it('should handle different resource types', async () => {
      ...
      for (const resource of resources) {
        const result = await caller.hasPermission({ action: 'read', resource })
        expect(typeof result).toBe('boolean')
        expect(result).toBe(false)
      }
    })
    ...
    it('should handle complex permission patterns', async () => {
      ...
      for (const permission of permissionTests) {
        const result = await caller.hasPermission(permission)
        expect(typeof result).toBe('boolean')
      }
    })
    ...
    it('should handle edge cases', async () => {
      ...
      // 测试空字符串
      const result1 = await caller.hasPermission({ action: '', resource: 'posts' })
      expect(result1).toBe(false)

      // 测试特殊字符
      const result2 = await caller.hasPermission({ action: 'read:*', resource: 'posts:*' })
    ...
    it('should handle hierarchical permissions', async () => {
      ...
      for (const test of hierarchicalTests) {
        const result = await caller.hasPermission(test)
        expect(typeof result).toBe('boolean')
        expect(result).toBe(false)
      }
    })
  ...
  describe('输入验证', () => {
    it('should validate permission check parameters', async () => {
      ...
      // 测试缺少必需字段
      await expect(caller.hasPermission({ action: 'read' } as unknown)).rejects.toThrow()

      await expect(caller.hasPermission({ resource: 'posts' } as unknown)).rejects.toThrow()
    })
    ...
    it('should handle invalid parameter types', async () => {
      ...
      // 测试无效类型
      await expect(
        caller.hasPermission({ action: 123, resource: 'posts' } as unknown)
      ).rejects.toThrow()

      await expect(
        caller.hasPermission({ action: 'read', resource: 456 } as unknown)
      ).rejects.toThrow()
    })
    ...
    it('should handle null and undefined values', async () => {
      ...
      await expect(
        caller.hasPermission({ action: null, resource: 'posts' } as unknown)
      ).rejects.toThrow()

      await expect(
        caller.hasPermission({ action: 'read', resource: null } as unknown)
      ).rejects.toThrow()
    })
  })
  ...
  describe('权限检查逻辑', () => {
    ...
    it('should allow access for authenticated users', async () => {
      ...
      // 已认证用户可以访问受保护端点
      await expect(caller.getUser()).resolves.toBeDefined()
      await expect(caller.getPermissions()).resolves.toEqual([])
      await expect(caller.hasPermission({ action: 'read', resource: 'posts' })).resolves.toBe(false)
    })
  })
  ...
  describe('类型安全性', () => {
    it('should enforce type safety for all operations', () => {
      ...
      // 编译时类型检查
      expect(typeof caller.getSession).toBe('function')
      expect(typeof caller.isAuthenticated).toBe('function')
      expect(typeof caller.getUser).toBe('function')
      expect(typeof caller.getPermissions).toBe('function')
      expect(typeof caller.hasPermission).toBe('function')
    })
