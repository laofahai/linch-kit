...
describe('@linch-kit/trpc CRUD Router', () => {
  ...
  describe('delete 删除操作', () => {
    ...
    it('should handle complex where conditions', async () => {
      ...
      const result = await caller.delete({
        model: 'User',
        where: {
          AND: [{ status: 'inactive' }, { lastLogin: { lt: new Date('2023-01-01') } }],
        },
      })
  ...
  describe('count 统计操作', () => {
    ...
    it('should handle complex where conditions', async () => {
      ...
      const result = await caller.count({
        model: 'User',
        where: {
          AND: [{ status: 'active' }, { role: { in: ['admin', 'moderator'] } }],
        },
      })
  ...
  describe('权限检查', () => {
    it('should require authentication for all operations', async () => {
      ...
      // 测试所有操作都需要认证
      await expect(caller.findMany({ model: 'User' })).rejects.toThrow('需要登录才能访问此资源')

      await expect(caller.create({ model: 'User', data: {} })).rejects.toThrow(
        '需要登录才能访问此资源'
      )

      await expect(caller.update({ model: 'User', where: { id: 1 }, data: {} })).rejects.toThrow(
        '需要登录才能访问此资源'
      )

      await expect(caller.delete({ model: 'User', where: { id: 1 } })).rejects.toThrow(
        '需要登录才能访问此资源'
      )

      await expect(caller.count({ model: 'User' })).rejects.toThrow('需要登录才能访问此资源')
    })
  ...
  describe('输入验证', () => {
    it('should validate model parameter', async () => {
      ...
      // 测试无效模型名称类型应该抛出错误
      await expect(caller.findMany({ model: 123 as unknown })).rejects.toThrow()
    })
    ...
    it('should validate data types', async () => {
      ...
      await expect(caller.create({ model: 'User', data: validData })).resolves.toEqual({})
    })
  ...
  describe('错误处理', () => {
    it('should handle invalid input gracefully', async () => {
      ...
      // 测试缺少必需字段
      await expect(caller.create({ model: 'User' } as unknown)).rejects.toThrow()

      await expect(caller.update({ model: 'User', data: {} } as unknown)).rejects.toThrow()
    })
