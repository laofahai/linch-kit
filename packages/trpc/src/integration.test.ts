...
import { appRouter, createLinchKitContext, createTRPCContext, type AppRouter } from './server'
...
describe('@linch-kit/trpc Integration Tests', () => {
  ...
  describe('完整 tRPC 应用路由器集成', () => {
    ...
    it('should handle concurrent requests to different routes', async () => {
      ...
      // 并发请求不同路由
      const promises = [caller.health.ping(), caller.health.status(), caller.system.info()]
  ...
  describe('CRUD 和 Auth 路由器集成', () => {
    ...
    it('should enforce authentication across CRUD operations', async () => {
      ...
      // 所有 CRUD 操作都应该要求认证
      await expect(crudCaller.findMany({ model: 'User' })).rejects.toThrow('需要登录才能访问此资源')

      await expect(crudCaller.create({ model: 'User', data: {} })).rejects.toThrow(
        '需要登录才能访问此资源'
      )
      ...
      await expect(crudCaller.delete({ model: 'User', where: { id: 1 } })).rejects.toThrow(
        '需要登录才能访问此资源'
      )

      await expect(crudCaller.count({ model: 'User' })).rejects.toThrow('需要登录才能访问此资源')
    })
  ...
  describe('上下文创建和管理集成', () => {
    ...
    it('should handle context inheritance across routers', async () => {
      ...
      const crudCaller = crudRouter.createCaller({
        ...context,
  ...
  describe('错误处理和边界情况集成', () => {
    it('should handle authentication errors consistently', async () => {
      ...
      await expect(authCaller.getUser()).rejects.toThrow('需要登录才能访问此资源')
    })
    ...
    it('should handle malformed requests gracefully', async () => {
      ...
      // 处理无效输入
      await expect(crudCaller.create({ model: 'User' } as unknown)).rejects.toThrow()

      await expect(crudCaller.hasPermission({ action: 'read' } as unknown)).rejects.toThrow()
    })
