...
export const authCoreConfigPlugin: ConfigPlugin = {
  ...
  async validateConfig(config: any) {
    try {
      ...
      // 验证 JWT 配置
      if (authConfig.session.strategy === 'jwt' && !process.env.NEXTAUTH_SECRET && !authConfig.session.secret) {
        errors.push({
  ...
  getConfigTemplate(type: 'ts' | 'js' | 'json' = 'ts') {
    switch (type) {
      case 'ts':
        return `import type { AuthConfig } from '@linch-kit/auth-core'
...
`
      ...
      case 'js':
        return `/** @type {import('@linch-kit/auth-core').AuthConfig} */
const authConfig = {
  providers: [
    // Add your authentication providers here
  ],
...
export function registerAuthCoreConfigPlugin() {
  ...
  if (typeof globalThis !== 'undefined' && (globalThis as any).__LINCH_CONFIG_REGISTRY__) {
    (globalThis as any).__LINCH_CONFIG_REGISTRY__.registerPlugin(authCoreConfigPlugin)
  }
}
