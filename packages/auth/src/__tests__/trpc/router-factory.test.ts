...
describe('tRPC Router Factory', () => {
  ...
  describe('中间件集成', () => {
    it('should handle auth middleware', async () => {
      ...
      await expect(authMiddleware(unauthorizedReq, mockRes, mockNext)).rejects.toThrow(
        'Unauthorized'
      )
    })
    ...
    it('should handle permission middleware', async () => {
      ...
      await expect(writeMiddleware(unauthorizedReq, {}, mockNext)).rejects.toThrow('Forbidden')
    })
  })
