...
describe('CacheManager', () => {
  ...
  describe('Batch Operations', () => {
    it('should handle multiple cache operations', async () => {
      ...
      // 执行批量获取
      const results = await Promise.all(operations.map(op => cacheManager.get(op.key)))
    ...
    it('should handle concurrent cache operations', async () => {
      ...
      // 并发设置
      await Promise.all(concurrentOps.map(op => cacheManager.set(op.key, op.value)))

      // 并发获取
      const results = await Promise.all(concurrentOps.map(op => cacheManager.get(op.key)))
  ...
  describe('Memory Management', () => {
    ...
    it('should handle many cache entries', async () => {
      ...
      // 获取所有条目
      const results = await Promise.all(entries.map(entry => cacheManager.get(entry.key)))
