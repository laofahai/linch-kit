...
describe('CrudManager Advanced Operations', () => {
  ...
  describe('Update Operations', () => {
    ...
    it('should throw error when updating non-existent record', async () => {
      ...
      await expect(crudManager.update('user', 'non-existent-id', updateData))
  ...
  describe('Transaction Operations', () => {
    ...
    it('should handle transaction with timeout option', async () => {
      ...
      const result = await crudManager.transaction(
        async (tx) => transactionResult,
        { timeout: 5000 }
      )
    ...
    it('should handle transaction rollback on error', async () => {
      ...
      await expect(crudManager.transaction(async (tx) => {
        throw error
  ...
  describe('Batch Operations', () => {
    it('should handle batch create operations', async () => {
      ...
      const results = await Promise.all(
        users.map(user => crudManager.create('user', user))
      )
    ...
    it('should handle batch delete operations', async () => {
      ...
      const results = await Promise.all(
        userIds.map(id => crudManager.delete('user', id))
      )
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors', async () => {
      ...
      await expect(crudManager.findMany('user'))
    ...
    it('should handle constraint violation errors', async () => {
      ...
      await expect(crudManager.create('user', { email: 'duplicate@example.com' }))
    ...
    it('should handle foreign key constraint errors', async () => {
      ...
      await expect(crudManager.create('post', { 
        title: 'Test Post', 
        ...
        getAll: mock(() => [
          {
            plugin: {
              ...
              hooks: {
                ...
                afterCreate: mock(),
                beforeUpdate: mock((entityName, id, data, existing, options) => ({ ...data, updated: true })),
                afterUpdate: mock(),
                beforeDelete: mock(),
                afterDelete: mock(),
    ...
    it('should handle plugin hook errors gracefully', async () => {
      const errorPlugin = {
        plugin: {
          ...
          hooks: {
            beforeCreate: mock(() => { throw new Error('Plugin error') })
          }
        },
        config: {},
        status: 'active',
      ...
      await expect(crudManagerWithPlugins.create('user', { email: 'test@example.com' }))
      ...
      const plugins = [
        {
          plugin: {
            ...
            hooks: {
              beforeCreate: mock(() => { executionOrder.push('plugin-1'); return {} })
            }
          },
          config: {},
          status: 'active',
        ...
        },
        {
          plugin: {
            ...
            hooks: {
              beforeCreate: mock(() => { executionOrder.push('plugin-2'); return {} })
            }
          },
          config: {},
          status: 'active',
  ...
  describe('Audit Log Integration', () => {
    ...
    it('should handle audit logging errors gracefully', async () => {
      ...
      mockPrisma.user.create.mockResolvedValue(createdUser)
      mockLogger.info.mockImplementation(() => { throw new Error('Audit log failed') })
