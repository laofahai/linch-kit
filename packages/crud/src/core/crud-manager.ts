...
export class CrudManager {
  ...
  async findMany<T>(
    ...
    const results = await (model as { findMany: (query: unknown) => Promise<T[]> }).findMany(prismaQuery)
  ...
  async findById<T>(
    ...
    return await this.findFirst<T>(entityName, {
  ...
  async update<T>(
    ...
    // 执行更新
    const result = await (model as { update: (args: { where: { id: string }, data: unknown }) => Promise<T> }).update({
      where: { id },
      data: processedData
    })
  ...
  async delete(
    ...
    if (entity?.options?.softDelete) {
      await (model as { update: (args: { where: { id: string }, data: unknown }) => Promise<unknown> }).update({
        where: { id },
        data: {
          deletedAt: new Date(),
  ...
  async count(
    ...
    return await (model as { count: (query: { where: unknown }) => Promise<number> }).count(countQuery)
  }
  ...
  async createMany<T>(
    ...
    return await (model as { createMany: (args: { data: unknown[], skipDuplicates?: boolean }) => Promise<{ count: number }> })
      .createMany({
        data,
