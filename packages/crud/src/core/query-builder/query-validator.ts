...
export class QueryValidator {
  ...
  validate(query: Record<string, unknown>, entity: Entity): QueryValidationResult {
    ...
    // 验证包含关系
    if (query.include) {
      const includeValidation = this.validateInclude(
  ...
  private validateCondition(
    ...
    if (typeof condition === 'object') {
      ...
      for (const op of operators) {
        ...
        // 验证操作符与字段类型的兼容性
        if (['contains', 'startsWith', 'endsWith'].includes(op) && fieldDef.type !== 'string') {
          errors.push({
            field,
        ...
        if (
          ...
          !['number', 'integer', 'float', 'date', 'datetime'].includes(
            (fieldDef as Record<string, unknown>).type as string
          )
        ) {
          errors.push({
            field,
        ...
        if (
          (op === 'in' || op === 'notIn') &&
          !Array.isArray((condition as Record<string, unknown>)[op])
        ) {
          errors.push({
            field,
  ...
  private calculateComplexity(query: Record<string, unknown>): number {
    ...
    if (query.orderBy) {
      ...
      const fields = Array.isArray(orderBy) ? orderBy.length : Object.keys(orderBy).length
      complexity += fields * 2
    }
  ...
  private calculateWhereComplexity(where: Record<string, unknown>, depth = 0): number {
    ...
    for (const [key, value] of Object.entries(where)) {
      if (key === 'AND' && Array.isArray(value)) {
        complexity += value.reduce(
          (sum, cond) => sum + this.calculateWhereComplexity(cond, depth + 1),
          0
        )
      } else if (key === 'OR' && Array.isArray(value)) {
        complexity +=
          value.reduce((sum, cond) => sum + this.calculateWhereComplexity(cond, depth + 1), 0) * 2
      } else if (key === 'NOT') {
        complexity +=
          this.calculateWhereComplexity(value as Record<string, unknown>, depth + 1) * 1.5
      } else {
        complexity += 1
      }
    }
