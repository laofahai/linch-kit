...
export class QueryOptimizer {
  ...
  optimize(query: Record<string, unknown>, entity: Entity): OptimizedQuery {
    ...
    // 分析ORDER BY
    if (query.orderBy) {
      const orderOptimization = this.optimizeOrderBy(
    ...
    // 分析关联查询
    if (query.include) {
      const includeOptimization = this.optimizeInclude(
    ...
    // 分析字段选择
    if (query.select) {
      const selectOptimization = this.optimizeSelect(
        query.select as Record<string, unknown>,
        entity
      )
  ...
  private getIncludeDepth(include: Record<string, unknown>, depth = 1): number {
    ...
    for (const value of Object.values(include)) {
      if (typeof value === 'object' && value !== null && 'include' in value) {
        const nestedDepth = this.getIncludeDepth(
          (value as Record<string, unknown>).include as Record<string, unknown>,
          depth + 1
        )
  ...
  private countSelectedFields(select: Record<string, unknown>): number {
    ...
    for (const value of Object.values(select)) {
      if (value === true) {
        ...
        count += this.countSelectedFields(
   ...
   */
  batchOptimize(
    queries: Array<{ query: Record<string, unknown>; entity: Entity }>
  ): Array<OptimizedQuery> {
