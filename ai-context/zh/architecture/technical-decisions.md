...
3. **包间集成**: @linch-kit/ui (基础) + @linch-kit/crud-ui (业务) + @linch-kit/auth-ui (认证)
...
2. **包间职责分离**:
   - **@linch-kit/ui**: 基础组件层，shadcn/ui 二次封装，提供简化调用方法
   - **@linch-kit/crud-ui**: CRUD 业务组件
...
原计划开发独立的 @linch-kit/auth-ui 包，但在实际开发过程中发现认证 UI 组件具有高度的业务定制性，不同项目对认证流程和界面有不同需求。
