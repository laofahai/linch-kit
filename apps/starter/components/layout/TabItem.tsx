...
import { Home, Settings, Users, Pin, PinOff, Circle, BarChart3, Zap } from 'lucide-react'
...
export function TabItem({ tab, isActive }: TabItemProps) {
  const { setActiveTab, removeTab, pinTab, unpinTab, closeOthers, updateTab } = useTabsStore()
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* 固定图标 */}
        {tab.pinned && <Pin className="h-4 w-4 flex-shrink-0 opacity-70" />}

        {/* 标题 */}
        <span className="truncate flex-1 text-left">{tab.title}</span>

        {/* 修改状态指示器 */}
        {tab.modified && <Circle className="h-2 w-2 fill-current flex-shrink-0" />}
      </button>
      ...
      <DropdownMenu open={isContextMenuOpen} onOpenChange={setIsContextMenuOpen}>
        ...
        <DropdownMenuContent align="start" className="w-48">
          ...
          <DropdownMenuItem onClick={handleRename}>重命名标签</DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={handleCloseOthers}>关闭其他标签</DropdownMenuItem>
          {tab.closable && (
