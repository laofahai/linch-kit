...
export const PluginEntity = defineEntity('Plugin', {
  // 基础信息
  name: defineField.string().required().unique().max(100).description('插件名称'),

  version: defineField.string().required().max(20).description('插件版本'),

  description: defineField.text().optional().description('插件描述'),

  authorName: defineField.string().required().max(100).description('插件作者名称'),
  ...
  // 状态管理
  status: defineField
    .enum(['draft', 'published', 'deprecated', 'removed'])
    .required()
    .default('draft')
    .description('插件状态'),

  isOfficial: defineField.boolean().default(false).description('是否为官方插件'),

  isFeatured: defineField.boolean().default(false).description('是否为推荐插件'),

  // 统计信息
  downloads: defineField.int().default(0).min(0).description('总下载次数'),
  ...
  // 关系字段
  author: defineField.relation('User').required().description('插件作者'),

  versions: defineField.relation('PluginVersion').oneToMany().description('插件版本历史'),
...
export const PluginVersionEntity = defineEntity('PluginVersion', {
  // 关系
  plugin: defineField.relation('Plugin').required().description('所属插件'),

  // 版本信息
  version: defineField.string().required().max(20).description('版本号'),

  changelog: defineField.text().optional().description('版本更新日志'),
  ...
  // 状态
  isStable: defineField.boolean().default(true).description('是否为稳定版本'),

  isLatest: defineField.boolean().default(false).description('是否为最新版本'),
...
export const TenantPluginEntity = defineEntity('TenantPlugin', {
  // 关系
  tenant: defineField.relation('Tenant').required().description('租户'),

  plugin: defineField.relation('Plugin').required().description('插件'),

  // 安装信息
  version: defineField.string().required().max(20).description('安装的版本'),
  ...
  updateChannel: defineField
    .enum(['stable', 'beta', 'alpha'])
    .default('stable')
    .description('更新通道'),

  // 错误信息
  lastError: defineField.text().optional().description('最后一次错误信息'),
