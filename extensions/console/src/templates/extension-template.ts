...
export function createExtensionTemplate(
  ...
  const template: ExtensionTemplate = {
    ...
    async initialize(instance: ExtensionInstance) {
      ...
      // 注册基础消息处理器
      communicationAPI.onMessage('ping', async (message) => {
        return { pong: true, timestamp: Date.now() }
      })
      
      // 注册状态查询处理器
      communicationAPI.onMessage('getStatus', async (message) => {
        return {
          name,
          status: 'running',
          timestamp: Date.now(),
          ...
          default: () => React.createElement('div', {
            ...
            React.createElement('div', { key: 'info' }, [
              ...
              React.createElement('ul', { key: 'info-list' }, [
                ...
                React.createElement('li', { key: 'category' }, `Category: ${metadata.category}`),
                React.createElement('li', { key: 'permissions' }, `Permissions: ${metadata.permissions.join(', ')}`)
              ])
            ])
          ])
        })),
      ...
      routeRegistry.addRoute({
        ...
        component: React.lazy(() => Promise.resolve({
          default: () => React.createElement('div', {
            ...
            React.createElement('p', { key: 'desc' }, 'Configure your extension settings here.'),
            React.createElement('div', { key: 'placeholder' }, 'Settings UI will be implemented here...')
          ])
        })),
        metadata: {
    ...
    async registerComponents(instance: ExtensionInstance) {
      // 这里可以注册可复用的组件
      // 例如：
 ...
 * 示例：简单的 Extension 实现
 */
export const exampleExtension = createExtensionTemplate(
  'example-extension',
  'Example Extension',
  {
    hasUI: true,
...
// 为通信示例添加额外的消息处理器
communicationExtension.initialize = async function(instance: ExtensionInstance) {
  ...
  communicationAPI.onMessage('requestStatus', async (message) => {
    const targetExtension = message.payload.target
    if (targetExtension) {
      try {
...
export class ExtensionTemplateFactory {
  ...
  static createFullExtension(name: string, displayName: string): ExtensionTemplate {
    return createExtensionTemplate(name, displayName, {
      ...
      hasAPI: true,
      hasSchema: true,
      permissions: ['ui:render', 'api:read', 'api:write', 'database:read', 'database:write', 'extension:communicate']
    })
  }
